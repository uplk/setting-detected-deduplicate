...
class Injector(metaclass=ABCMeta):
    ...
    def change_setting_before_run(self,event_count,strategy):
        ...
        elif strategy == "permission_lazy_1":
            self.permission_lazy_1()
            event = Event(None,"permission_lazy_1",self.devices[1],event_count)
    ...
    def network_immediate_1(self):
        ...
        device.adb_setting("network_immediate_1")

    def network_lazy_1(self):
        device = self.devices[1]
        if device.wifi_state:
            device.adb_setting("network_lazy_1_1")
            device.wifi_state = False
        elif not device.wifi_state:
            device.adb_setting("network_lazy_1_2")
            device.wifi_state = True

    def network_lazy_2(self):
        device = self.devices[1]
        if device.wifi_state:
            device.adb_setting("network_lazy_2_1")
            device.wifi_state = False
        elif not device.wifi_state:
    ...
    def location_lazy_1(self):
        ...
        if self.devices[1].adb_get_gps_state():
            self.devices[1].adb_setting("close_gps")
            self.devices[1].gps_state = False
        elif not self.devices[1].adb_get_gps_state():
            self.devices[1].adb_setting("open_gps")
    ...
    def location_lazy_2(self):
        ...
        orientation2=device1.orientation
        if self.devices[1].adb_get_gps_state():
            self.devices[1].adb_setting("gps_accuracy")
            if device0(text="AGREE").count > 0:
                device0(text="AGREE").click()
            self.devices[1].adb_setting("gps_device")
            self.devices[1].gps_state = False
        elif not self.devices[1].adb_get_gps_state():
            self.devices[1].adb_setting("gps_accuracy")
            if device0(text="AGREE").count > 0:
    ...
    #     #     self.devices[1].adb_setting("language_en")
    #     #     self.devices[1].language = "ch"
    #     # elif self.devices[1].language == "ch":
    #     #     self.devices[1].adb_setting("language_ch")
