...
from phones.injector_honor import  InjectorHonor
from phones.injector_emulator_8_1 import InjectorEmulator8_1


class Executor(object):

    def __init__(self, devices, app, strategy_list, pro_click, pro_longclick, pro_scroll,
                 pro_home, pro_edit, pro_naturalscreen, pro_leftscreen, pro_back, pro_splitscreen, emulator_path,
                 android_system,
                 root_path, resource_path, testcase_count, start_testcase_count, event_num, timeout, policy_name,
                 setting_random_denominator, serial_or_parallel, emulator_name, is_login_app, rest_interval, trace_path,
                 choice):
        ...
        if android_system == "honor":
            self.injector = InjectorHonor(devices=devices,
                                          app=app,
                                          ...
                                          choice=choice)
        elif android_system == "emulator8.1":
            self.injector = InjectorEmulator8_1(devices=devices,
                                          app=app,
                                          strategy_list=strategy_list,
                                          emulator_path=emulator_path,
                                          android_system=android_system,
    ...
    def get_policy(self):
        if self.policy_name == "random":
            ...
            policy = RandomPolicy(self.devices, self.app, self.emulator_path, self.android_system, self.root_path,
                                  self.pro_click, self.pro_longclick, self.pro_scroll, self.pro_edit,
                                  self.pro_naturalscreen, self.pro_leftscreen, self.pro_back, self.pro_splitscreen,
                                  self.pro_home)
        else:
    ...
    def execute_event(self, device, event, num):
        try:
            ...
            if event.action in have_view_action and event.view is not None:
                if device.use(resourceId=event.view.resourceId).count == 0 or device.use(
                        className=event.view.className).count == 0:
                    lines = device.use.dump_hierarchy()
                    if (event.view.resourceId != "" and event.view.resourceId not in lines) or (
        ...
        for line in lines:
            ...
            if "Start::" in line:
                ...
                if record_flag == True:
                    ...
                    record_flag = False
                    f_read_trace.close()
                    self.utils.generate_html(os.path.join(self.error_path, linelist[1]),
                                             os.path.join(self.error_path, linelist[1]), linelist[1])
            elif line.strip() != "":
                print("-----------------------" + '\n' + line)
                ...
                if elementlist[1] == "save_state":
                    ...
                    time.sleep(self.rest_interval * 1)
                if "device1" in line and "save_state" in line and self.devices[0].state is not None and self.devices[
                    1].state is not None and not self.devices[0].state.same(self.devices[1].state):
                    print("different!")
    ...
    def clear_and_restart_app(self, event_count, strategy):
        ...
        # special deal for ByteDance's APP-TikTok
        if self.app.package_name == "com.ss.android.ugc.aweme":
            self.injector.dy_init()
            ...
            while event_count < self.event_num:
                ...
                if self.devices[0].last_state is not None and change_flag:
                    ...
                    if not self.devices[0].state.same(self.devices[1].state):
                        ...
                        # write wrong
                            ...
                            print("Write wrong!")
                            self.utils.write_error(1, run_count, self.devices[1].wrong_event_lists,
                                                   self.devices[1].f_wrong, self.devices[1].wrong_num)
                ...
                # other devices cannot execute choice event due to setting, record fail, skip this loop
                    ...
                    if not self.checkduplicate():
                        print("write error")
                        self.utils.draw_error_frame()
                        self.utils.write_error(fail_device, run_count, self.devices[fail_device].error_event_lists,
                                               self.devices[fail_device].f_error, self.devices[fail_device].error_num)
